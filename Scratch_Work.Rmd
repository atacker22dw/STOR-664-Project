...
reg1<-lm(Gpm~sqrtcase+SAHG+BG)
summary(reg1)

reg2<-lm(Rpm~sqrtCase+BR)
summary(reg2)
...
Having now added in binary variables for a stay at home order and bar closure mandates, the adjusted R-squared has improved only ever so slightly.  I was not able to add the binary variable for stay at home in the Raleigh data set because we are missing data on almost the entirety of that period (in all likelihood this stay at home order is precisely why we don't have data on this period).  That said, the two positive/interesting results to come out of this (slightly) more inclusive model, is a significantly negative coefficient for cases in Raleigh and a significantly positive one in Greenville.  This would seem to coincide with our intuition about lax regulations.  More over, in Greenville bar closures had a marginally significant negative effect on particulate matter while stay at home orders were largely inconsequential.  This could be because a state mandated bar closure is more feasibly enforced than a stay at home order.  Bars were actually shut down and it was not left to individual discretionary autonomy as to whether to adhere to bar closures as it was stay at home orders.  
